...
class ModelManager(nn.Module):
    ...
    def __init__(self, args, num_word, num_slot, num_intent, global_g):
        ...
        self.__intent_decoder = nn.Sequential(
            ...
            nn.Linear(self.__args.encoder_hidden_dim + self.__args.attention_output_dim, self.__args.intent_embedding_dim),
        )
        ...
        self.slot_embedding = nn.Parameter(
            torch.FloatTensor(self.__num_slot, self.__args.intent_embedding_dim))  
        ...
        self.__slot_decoder = LSTMDecoder(
            ...
            self.__args.encoder_hidden_dim + self.__args.attention_output_dim,
            self.__args.slot_decoder_hidden_dim,
            self.__args.intent_embedding_dim, self.__args.dropout_rate)
        
        self.global_gnn = heterogeneousGNN(self.__args.intent_embedding_dim, self.global_g)
    ...
    def forward(self, text, seq_lens, n_predicts=None):
        ...
        int_res = self.intent_embedding
        slot_res = self.slot_embedding
        for i in range(3):
            slot_embed, int_embed = self.global_gnn(int_res, slot_res, self.global_g)
            # slot_embed = self.dropout(slot_embed)
            # int_embed = self.dropout(int_embed)
            slot_embed = F.dropout(slot_embed, p=0.1, training=self.training)
            int_embed = F.dropout(int_embed, p=0.1, training=self.training)
            slot_embed = F.normalize(slot_embed)
            int_embed = F.normalize(int_embed)
            slot_res = torch.add(slot_res, slot_embed)
            int_res = torch.add(int_res, int_embed)
        ...
        pred_intent = torch.matmul(pred_intent, int_res.T)
        seq_lens_tensor = torch.tensor(seq_lens)
        if self.__args.gpu:
            seq_lens_tensor = seq_lens_tensor.to(self.__device)
        ...
        global_dgl_graphs = []
        for i in range(len(seq_lens)):
            slot_dgl_graphs.append(dgl.from_scipy(sp.coo_matrix(slot_adj[i].cpu())))
            global_dgl_graphs.append(dgl.from_scipy(sp.coo_matrix(global_adj[i].cpu())))
        slot_dgl_batch = dgl.batch(slot_dgl_graphs).to(self.__device)
        global_dgl_batch = dgl.batch(global_dgl_graphs).to(self.__device)
        pred_slot = self.__slot_decoder(
            slot_lstm_out, seq_lens,
            global_adj=global_dgl_batch,
            slot_adj=slot_dgl_batch,
            intent_embedding=int_res,
            slot_embedding=slot_res
        )
        if n_predicts is None:
            return F.log_softmax(pred_slot, dim=1), pred_intent, self.global_gnn.graph_kg_loss(int_res, slot_res, self.global_g)
        else:
...
class LSTMDecoder(nn.Module):
    ...
    def __init__(self, args, input_dim, hidden_dim, output_dim, dropout_rate, embedding_dim=None, extra_dim=None):
        ...
        self.__slot_graph = MultiHeadGATLayer(
            self.__hidden_dim,
        ...
        self.__global_graph = MultiHeadGATLayer(
            self.__hidden_dim,
            self.__args.decoder_gat_hidden_dim,
            self.__hidden_dim,
            self.__args.gat_dropout_rate, self.__args.alpha, self.__args.n_heads,
    ...
    def forward(self, encoded_hiddens, seq_lens, global_adj=None, slot_adj=None, intent_embedding=None, slot_embedding=None):
        ...
        out = torch.matmul(out, slot_embedding.T)
