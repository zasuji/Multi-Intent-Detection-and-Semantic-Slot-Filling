    ...
    def train(self):
        ...
        for epoch in range(0, self.__dataset.num_epoch):
            ...
            for text_batch, slot_batch, intent_batch in tqdm(dataloader, ncols=50):
                ...
                if self.args.gpu:
                    text_var = text_var.to(self.device)
                    slot_var = slot_var.to(self.device)
                    intent_var = intent_var.to(self.device)

                random_slot, random_intent = random.random(), random.random()

                slot_out, intent_out, kg_loss = self.__model(text_var, seq_lens)
                ...
                print(slot_loss_alpha * slot_loss, intent_loss_alpha * intent_loss, kg_loss)
