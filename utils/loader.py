...
class DatasetManager(object):
    ...
    def __init__(self, args):
        ...
        self.__args = args
        self.__device = torch.device("cuda:" + str(args.gpu_id))
    ...
    def quick_build(self):
        ...
        self.create_graph(self.__text_slot_data['train']+self.__text_slot_data['dev']+self.__text_slot_data['test'], self.__text_intent_data['train']+self.__text_intent_data['dev']+self.__text_intent_data['test'])
    ...
    def create_graph(self, slot, intent):
        # 需要在这里构建图的开始与结束点
        # 先不计频率的去记录获得的图中的关系
        # 首先先做单向的关系图
        g_star = []
        g_end = []
        g_rela = []
        for i in tqdm(range(len(slot)),desc='Constructing heterogeneous graph'):
            int = []
            sl_B = []
            # 这里是int2int只考虑从前到后
            if '#' in intent[i][0]:
                for element in intent[i][0].split('#'):
                    int.append(self.intent_alphabet.instance2index[element]+len(self.slot_alphabet))
                g_star = g_star + [i[0] for i in list(combinations(int, 2))]
                g_end = g_end + [i[1] for i in list(combinations(int, 2))]
                g_rela = g_rela + [0]*len(list(combinations(int, 2)))
            else:
                int.append(self.intent_alphabet.instance2index[intent[i][0]]+len(self.slot_alphabet))      
            # 这里将单向的slot'B'2slot'I'加入
            for j in range(len(slot[i])):
                if slot[i][j][0]=='B':
                    sl_B.append(self.slot_alphabet.instance2index[slot[i][j]])
                if j>=1 and slot[i][j][0]=='I' and slot[i][j-1][0]=='B':
                    g_star.append(self.slot_alphabet.instance2index[slot[i][j-1]])
                    g_end.append(self.slot_alphabet.instance2index[slot[i][j]])
                    g_rela.append(2)
            # 这里将单向的int2slot'B'加入
            g_end = g_end + sl_B*len(int)
            g_rela = g_rela + [1]*len(sl_B)*len(int)
            for i in int:
                g_star = g_star + [i]*len(sl_B)

        combination_counters = Counter([(a,b) for a,b in zip(g_star, g_end)])
        appear_time = [combination_counters[i] for i in combination_counters]

        # 这里经过调试通过观察appear_time中不同的结点队出现的次数，选择20和130作为分割的点
        # 出现次数在不足20次的连接关系直接消除，连接关系在[20,130)之间的记作一种关系，大于等于130的记为另一种
        # 于此首先将只有单向的图进行基于出现频率的区分
        appear_time1 = [i for i in appear_time if i>=20]
        combinations1 = [i for i in combination_counters if combination_counters[i]>=20]
        # combinations1 = combination_counters
        g_star = [i[0] for i in combinations1]
        g_end = [i[1] for i in combinations1]
        g_rela_ex = [0 if i<130 else 1 for i in appear_time1]
        g_rela = [a*2+b for a,b in zip(g_rela,g_rela_ex)]

        # 至此已经把int2int,int2slot'B',slot'B'2slot'A'的单向异构图的开始结束节点与边的关系已经登记完毕了
        # 所以我们应该下一步就要开始去考虑双向的关系，并且把图给构建出来了
        g_rela = g_rela + [max(g_rela)+1+i for i in g_rela]
        t = g_star
        g_star = g_star + g_end
        g_end = g_end + t
        global_g = dgl.graph((g_star,g_end))
        global_g.edata['type'] = torch.LongTensor(g_rela)
        global_g.ndata['id'] = torch.arange(len(self.slot_alphabet)+len(self.intent_alphabet), dtype=torch.long)
        self.global_g = global_g.to(self.__device)
